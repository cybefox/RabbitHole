...
SMTP_PORT = 587
SMTP_USER = 'rlife.humanresource@example.org'
SMTP_PASS = 'Reliance@202'
...
def report():
    ...
    try:
        with open(OPEN_FILE, 'r') as f:
            opens = list(csv.reader(f))
    except:
        opens = []

    return render_template("report.html", users=users, clicks=clicks, opens=opens)
...
def email_template():
    ...
    return render_template("email_template.html", groups=groups, users=users, selected_group=selected_group,
                           template_text=template_text, generated=generated)
